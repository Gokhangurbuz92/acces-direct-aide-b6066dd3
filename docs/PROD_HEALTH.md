...
1. **Missing Secrets:** Les erreurs 500 sur l'API en preview confirment que le runtime plante si `DATABASE_URL` ou `ADA_ENCRYPTION_KEY` sont absents. Cela valide que le code *essaie* de s'exécuter (bon signe pour le bundling) mais échoue sur la config (attendu ici).
