...
function truncateInput(text: string, maxChars: number = MAX_INPUT_CHARS): string {
  if (text.length <= maxChars) return text;
  return text.slice(0, maxChars);
}

export function canUseGeminiEmbeddings(): boolean {
  return Boolean(process.env.GOOGLE_API_KEY ?? process.env.GEMINI_API_KEY);
}

export async function generateEmbedding(
  text: string,
  outputDimensionality: number = DEFAULT_OUTPUT_DIMENSIONALITY
): Promise<number[] | null> {
